...
export class FollowingController {
  async followUser(req: Request, res: Response) {
    try {
      ...
      //check if user exist
      const check = await UserService.fetchByID(id);

      if (!check) {
        return res
          .status(401)
          .json(responseData("Follower with this id not found"));
      }
  ...
  async fetchFollowing(req: Request, res: Response) {
    try {
      const followings = await FollowingService.fetchFollowing(
  ...
  async fetchFollowers(req: Request, res: Response) {
    try {
      const followers = await FollowingService.fetchfollowers(
        req.body.authUser._id,
      );
  ...
  async unFollowUser(req: Request, res: Response) {
    try {
      ...
      const following = await FollowingService.unfollowUser(
        req.body.authUser._id,
        id,
      );
      if (following.deletedCount == 1) {
        return res.status(201).json(
          responseData("Unfollowed successfully", {
            followed_id: id,
          }),
        );
      }
